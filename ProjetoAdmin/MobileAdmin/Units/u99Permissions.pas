...
type
    ...
    public
        ...
        function VerifyLocation(): boolean;
...
function T99Permissions.VerifyLocation(): boolean;
begin
    Result := false;
    ...
    {$IFDEF ANDROID}
      ...
      {$IF CompilerVersion >= 35 }
        if TJBuild_VERSION.JavaClass.SDK_INT > 32 then
            Result := PermissionsService.IsEveryPermissionGranted([pCamera, pReadStorage])
        else
            Result := PermissionsService.IsEveryPermissionGranted([pCamera, pWriteStorage]);
      {$ELSE}
